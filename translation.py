...
def generate_sentences(language):
    ...
    language = language.lower()
    if language not in model_loaders:
        return None, None  # Return None for both if language is unsupported
    ...
    inputs = tokenizer(input_prompt, return_tensors="pt", truncation=True, max_length=50)
    
    # Set pad_token_id to eos_token_id if not defined for the model
